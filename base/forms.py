...
class InfluencerRegistrationForm(FlaskForm):
    ...
    profile_url = StringField('Profile Url', validators=[DataRequired(), Length(min=2, max=50)], render_kw={"placeholder": "E.g. www.instagram.com/abc"})
...
class SponsorRegistrationForm(FlaskForm):
    ...
    website = StringField('Website', validators=[DataRequired(), Length(min=2, max=50)], render_kw={"placeholder": "Company's Website E.g. www.abc.com"})
    email = StringField('Email', validators=[DataRequired(), Email()], render_kw={'placeholder': "Email"})
    picture = FileField('Update Profile Picture', validators=[FileAllowed(['jpg', 'png'])])
    submit = SubmitField('Update')
    bio = TextAreaField('Bio', validators=[Length(min=2, max=300)], render_kw={'placeholder': "Tell us more about you"})
